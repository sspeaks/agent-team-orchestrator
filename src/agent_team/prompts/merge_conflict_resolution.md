...
For hosted pull request conflicts, the orchestrator has recreated the isolated workspace from the PR branch and merged the latest target branch to materialize conflict markers. Resolve those markers locally; do not push, comment on the PR, or merge into the target branch. Recommend `ready_for_validation` when conflict resolution is complete so validation, review, merge approval, and PR branch update can run through the normal safety path.
