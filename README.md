...
The first supported providers are GitHub and Azure DevOps Services. GitHub remotes are detected from common SSH and HTTPS forms such as `git@github.com:owner/repo.git` and `https://github.com/owner/repo.git`; PR creation uses the `gh` CLI and your existing `gh auth login` state. Azure DevOps Services remotes are detected from `dev.azure.com`, `visualstudio.com`, and `ssh.dev.azure.com:v3` repository URLs; PR creation uses `az repos pr ...`, so install and authenticate the Azure DevOps CLI extension before using that provider. The selected remote's fetch URL and configured push URL must both resolve to the same supported provider repository, even when they use different SSH/HTTPS forms; mismatched, unsupported, or credential-bearing remote URLs block before any remote probe or push. Use Git credential helpers or authenticated SSH remotes instead of embedding tokens/passwords in remote URLs. Azure DevOps PR descriptions are passed to `az repos pr create` as a command-line argument and are capped at 24,000 UTF-8 bytes to avoid OS argument-length failures. Azure DevOps Server/on-prem and unknown providers are unsupported for PR mode.
