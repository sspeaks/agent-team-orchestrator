...
- Merge operations are serialized per source Git repository with a local file lock under `$AGENT_TEAM_HOME/locks`, so concurrent workers do not run multiple `git checkout`/`git merge` operations against the same source checkout at the same time.
...
- Merge approval is also an explicit human gate: successful review lands in `awaiting_merge_approval`, and the worktree is not merged or cleaned up until you run `issue approve-merge` and then run the merge phase.
...
Worktrees are persistent by design. Validation, review, merge, and merge-conflict-resolution runs for the same issue require and reuse the implementation-created worktree instead of resetting it, so agent changes remain available until a human approves the merge. Successful implementation and merge-conflict-resolution iterations are committed in the isolated worktree before validation, which keeps review/rework loops visible in Git history. If review rejects an implementation and sends the issue back to `ready_for_implementation`, the orchestrator first tries to merge the recorded source branch into the clean issue worktree. A clean source sync keeps the issue in `ready_for_implementation`; a detached-source worktree skips this mid-loop sync; and source-sync conflicts leave markers in the isolated worktree, write `merge.md`, and route to `ready_for_merge_conflict_resolution` before implementation rework continues. Snapshot commit subjects summarize the change, while commit bodies retain orchestration metadata (Issue, Phase, Run ID, Next Phase, and summary details). Workspace metadata is written to each issue's `workspace.json` artifact and is shown by `agent-team issue show` and the web issue detail page. Issue detail surfaces also show phase artifacts and logs, including merge-conflict-resolution reports when they exist. If a repo-backed issue is manually advanced to a post-implementation phase without workspace metadata, that phase blocks instead of creating a misleading new workspace.

Before first worktree creation, the source checkout must be clean because uncommitted or untracked files are not copied into a Git worktree. Commit or stash local changes before running the issue. Created worktrees are detached at the source `HEAD`; after review passes, use the web issue detail page's Open in VS Code button when workspace metadata exists or inspect the workspace path, then run `agent-team issue approve-merge <id>` to record human approval. The next merge run preserves the isolated-worktree iteration commits with a `--no-ff` source merge, adds a shortlog to the merge commit body, writes `merge.md`, archives `workspace.merged.json`, and removes the worktree only after the merge succeeds. If uncommitted issue-worktree changes remain at merge time, the merge run creates a final workspace snapshot commit as a safety net rather than the normal implementation history path. If the target branch cannot be inferred from `workspace.json`, pass `--branch <name>` to `approve-merge`.
...
If the final approved merge detects conflicts, it leaves the conflict markers in the isolated worktree, writes `merge.md`, and moves the issue to `ready_for_merge_conflict_resolution`. Run the next ready issue to let Copilot resolve the conflicts in that worktree. Conflict fixes write `merge_conflict_resolution.md`, then go back through validation, review, merge approval, and merge before the issue can close. Review-rejection source-sync conflicts use the same conflict-resolution phase and artifact name, but `merge.md` identifies them as workspace source-sync conflicts; after resolving those markers, Copilot returns to implementation when the prior review still requires code changes, otherwise to validation.
...
- answer human-input requests, approve plans, approve worktree merges, stop open issues, and manually transition issues through the existing state-machine validation
